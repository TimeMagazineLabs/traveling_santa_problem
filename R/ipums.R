...
housing_fields[1,] <- list(code="HC01_EST_VC02", key="total")
housing_fields[2,] <- list(code="HC01_EST_VC17", key="under_18")
housing_fields[3,] <- list(code="HC01_EST_VC27", key="one_unit")
housing_fields[4,] <- list(code="HC01_EST_VC28", key="two_plus_units")
housing_fields[5,] <- list(code="HC01_EST_VC29", key="mobile")

census <- load_census("../sources/ACS_15_1YR_S1101/ACS_15_1YR_S1101_with_ann.csv", housing_fields)

# IPUMS
data <- read.spss('../sources/ipums_ACS_2015.sav', to.data.frame=TRUE)
...
data$AGE_N <- data$YEAR - data$BIRTHYR
...
data$HH_ID <- paste(data$YEAR, data$DATANUM, data$SERIAL, sep='_')

# Let's do some immediate spot-checking

# This perfectly matches Census
print(paste("Total population is", prettyNum(sum(data$PERWT), big.mark=","))) 

# reduce ipums to unique households with the expected group quarter status
households <- data[!duplicated(data$HH_ID),]
...
data <- subset(data, data$GQ == "Households under 1970 definition")

children_under_18 <- subset(data, data$AGE_N < 18)
# One can check this against Census table B09001 (ACS 2015 1yr) if so desired. 
uniques_under_18  <- children_under_18[!duplicated(children_under_18$HH_ID), ] 
sum(children_under_18$PERWT)
print(paste("Households with those under 18:", sum(uniques_under_18$HHWT), "vs.", census[1,]$total * census[1,]$under_18 / 100))

# But we think Santa cuts you off at 18, and are only considering the continental US
all_children <- subset(data, data$AGE_N < 18 & data$STATEFIP!="Alaska" & data$STATEFIP!="Hawaii")

print(sum(all_children$PERWT))

# 92 percent celebrate Christmas: http://www.pewresearch.org/fact-tank/2015/12/21/5-facts-about-christmas-in-america/
all_children$i_celebrate <- (sample(1:100, NROW(all_children), replace=T) <= 92)
table(all_children$i_celebrate) / NROW(all_children)

children <- subset(all_children, all_children$i_celebrate == T)
print(paste(NROW(all_children) * 0.92, NROW(children)))

total_under_18 <- sum(children$PERWT)

print(paste("Total_present_recipients:", prettyNum(total_under_18, big.mark=",")))

#reduce to HHID
unique_households <- children[!duplicated(children$HH_ID),] 
total_households  <- sum(unique_households$PERWT)

#run these if you want to double check , should return 'character(0)'
unique_test <- unique(children$HH_ID)
setdiff(unique_test, unique_households$HH_ID)

states <- unique(children$STATEFIP)
...
national_children  <- children
national_children_households <- national_children[!duplicated(national_children$HH_ID),] 
